...
const { noCache, urlToArray, urlToPath } = require('./utils');
...
router.use((req, res, next) => {
  ...
  User.findOne({ username }).then(user => {
    ...
    // /me/b1.* - allowed
    // /me/b1[* - allowed
    // /me/b1abc - not allowed
    const match = user.public.find(_ => {
      return path == _ || path.startsWith(`${_}/`) || path.startsWith(`${_}.`) || path.startsWith(`${_}[`);
    });
...
router.get('/*', cors, noCache, (req, res, next) => {
  ...
  if (res.locals.restricted) {
    ...
    if (!path.startsWith(res.locals.restricted)) {
...
router.delete('/*', (req, res) => {
  ...
  user.dirty('storeJson', { method: 'DELETE', path: path.join('.') });
  user.save().then(user => {
