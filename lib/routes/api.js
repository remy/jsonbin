...
router.use((req, res, next) => {
  ...
  if (req.user && req.params.username === req.user.username) {
    // redirect if we're missing the trailing slash on the username
    if (req.path === `/${req.user.username}`) {
      // return res.redirect(`/${req.user.username}/`);
      console.log('modify path');
      req.path = `/${req.user.username}/`;
    }
    return next();
  }
  ...
  const username = req.path.split('/').filter(Boolean).shift();

  if (!username) {
    return next();
  }

  const path = urlToArray(req.path.split('/').slice(1)).join('.');
