...
schema.methods.generateBearer = function ({ path = '', expiresIn = '1 hour'} = {}) {
...
schema.statics.findOrCreate = ({ githubId = null, email } = {}, data) => {
  return User.findOne({ githubId }).then(user => {
    ...
    if (!body.store) {
      user.store = {
        gettingStarted: `check out the *help*, and try 'curl ${process.env.HOST}/me -H "authorization: token ${body.apikey}"'`,
